...
\begin{figure}[H]
	...
	\vspace{-0.5em}
	\caption{Detecção dos conflitos de dados para o registo A, referente ao andar de EX/MEM.}
	\vspace{-0.8em}
\end{figure} 
...
\begin{figure}[H]
	...
	\vspace{-0.5em}
	\caption{Circuito que permite verificar se uma instrução está associada à leitura do registo A.}
	\vspace{-0.8em}
\end{figure} 
...
\begin{figure}[H]
	...
	\vspace{-0.5em}
	\caption{Circuito que permite verificar se uma instrução está associada à leitura do registo B.}
	\vspace{-0.8em}
\end{figure} 
...
\begin{figure}[H]
	...
	\vspace{-0.5em}
	\caption{Detecção dos conflitos de dados para o registo A, referente ao andar de WB.}
	\vspace{-0.8em}
\end{figure} 
...
Ao analisar o esquema anterior, definiu-se dois sinais de controlo importantes na selecção do PC, \texttt{MUX\_BrPred} e \texttt{MUX\_NEXTPC}. 
...
\begin{figure}[H]
	...
	\vspace{-0.5em}
	\caption{Esquema lógico do sinal \texttt{MUX\_BrPred}.}
	\vspace{-0.8em}
\end{figure}

O sinal \texttt{MUX\_NEXTPC} tem a função de seleccionar o PC a ser endereçado na ROM, existem 4 possibilidades - o PC gerado pelo sinal de controlo anterior, o salto condicional proveniente do andar ID/OF \texttt{Jump\_BTB}, o salto incondicional proveniente do andar ID/OF \texttt{Jump\_Incond} ou o PC de \textit{backup}. De seguida está implementado a lógica do sinal \texttt{MUX\_NEXTPC}.

\begin{figure}[H]
	\centering
	\includegraphics[keepaspectratio=true, scale=0.27]{imagens/MuxNEXTPC}
	\vspace{-0.5em}
	\caption{Esquema lógico do sinal \texttt{MUX\_NEXTPC}.}
	\vspace{-0.8em}
\end{figure}

A actualização é controlada pelo sinal de \textit{enable} da tabela BTB, \texttt{we\_BTB}, este sinal verifica se ocorre um salto \textit{not taken} e se o salto for \textit{taken}, actualiza a tabela BTB com os novos valores de perdição, ou seja, é guardado na tabela a \textit{tag}, os 3 \textbf{bits} mais significativos do \textit{program conter}, o salto calculado no ID/OF, \texttt{Jump\_BTB}, e o \textit{bit} de predição é colocado a 1 bem como o \textit{bit} de validação. Se o salto previsto for \textit{taken} e na realidade for \textit{not taken}, a actualização da BTB é feita de forma igual com a diferença do \textit{bit} de predição a 0.
...
\begin{figure}[H]
	\centering
	\includegraphics[keepaspectratio=true, scale=0.27]{imagens/weaddrBTB}
	\vspace{-0.5em}
	\caption{Sinais de escrita da BTB.}
	\vspace{-0.8em}
\end{figure} 
