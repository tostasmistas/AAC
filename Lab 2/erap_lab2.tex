...
Pretende-se agora obter as componentes da tensão $v_{BE}$ e da corrente $I_{C}$ nas frequências $f_{OL}$, $f_{RF}$ e $f_{FI}$. \unsure{isto é para fazer? nos so tiramos do PFR}
...
Pode-se também obter o valor do ganho de compressão a -1 dB do ganho de transdução de conversão, ou seja, registar o valor de \todo{o que e o Pin} para o qual o ganho diminuiu 1 dB, ou seja, para o qual o ganho é 19 dB. 
...
O novo valor encontrado para $ P_{\text{OLDISP}}\left(\omega_{OL}\right) $ na Figura \ref{fig:GT_20} é de \unsure{3.5?} e, como se pode verificar na Figura \ref{fig:Imp_OL_RF}, representa um valor de $ Z_{\text{in}_{RF}} $ cuja componente real é positiva, 2.191 -j14.026 $\Omega $.
