...
Pretende-se agora obter as componentes da tensão $v_{BE}$ e da corrente $I_{C}$ nas frequências $f_{OL}$, $f_{RF}$ e $f_{FI}$. \todo{diz que isto tambem e para fazer}

Pode-se também obter o valor do ponto de compressão a -1 dB do ganho de transdução de conversão, ou seja, registar o valor de \todo{o que e o Pin} para o qual o ganho diminuiu 1 dB, ou seja, para o qual o ganho é 19 dB. 
...
O novo valor encontrado para $ P_{\text{OLDISP}}\left(\omega_{OL}\right) $ na Figura \ref{fig:GT_20} é de \unsure{3.5 ou 5?} e, como se pode verificar na Figura \ref{fig:Imp_OL_RF}, representa um valor de $ Z_{\text{in}_{RF}} $ cuja componente real é positiva, 2.191 -j14.026 $\Omega $.
...
\todo{Faz uma tabela igual à tabela \ref{tab:cap_ind} bobine ch1 e condensador1, são os que estao na imagem 2.223nH e 3.86pF, como valores usados mete 2.2nH e 3.9pF} \unsure{como e esta tabela?}
...
\begin{figure}[h]
	\centering
	\subfloat[]{\includegraphics[keepaspectratio=true, scale=0.35]{exps/EP_2_RF}}
	\hspace{8mm}
	\subfloat[]{\includegraphics[keepaspectratio=true, scale=0.33]{exps/EP_2_FI}}
	\vspace{-0.8em}
...
\begin{figure}[h]
	\centering
	\subfloat[]{\includegraphics[keepaspectratio=true, scale=0.35]{exps/EP_3_RF}}
	\hspace{8mm}
	\subfloat[]{\includegraphics[keepaspectratio=true, scale=0.33]{exps/EP_3_FI}}
	\vspace{-0.8em}
