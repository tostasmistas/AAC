...
O ganho de transdução, $G_T$, é definido como o rácio entre a potência entregue à carga (\textit{load}) e a potência disponível da fonte (\textit{source}). Quando numa situação de adaptação conjugada simultânea $G_T = \text{MAG}$.

Teoricamente, o factor de reflexão na saída deve ser infinito,  
\todo{teoria}
